...
class LevelSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    try {
      ...
      console.warn("Audio unlock pending");
    }
    ...
    // FIX: Perfected map height calculation to prevent black screen at the bottom
    ...
    // 5. Construct Popup UI
    this.createPopupUI();
    
    // 6. Setup Drag Scrolling
    this.setupScrolling();

    // 7. Auto-scroll to Current Level
    ...
    if (isUnlocked && !isCurrent) {
      for (let s = 0; s < 3; s++) {
        ...
        let star = this.add.text(-22 + (s * 22), 26, '★', {
          fontSize: '20px', color: starColor, stroke: '#111827', strokeThickness: 3
        }).setOrigin(0.5);
  ...
  createPopupUI() {
    ...
    // Scale panel dynamically to fit narrow phones safely
    let panelW = Math.min(320, GAME_WIDTH * 0.9);
    let panel = this.add.image(0, 0, 'popup_level_select').setDisplaySize(panelW, 340);
    ...
    this.closeBtn = this.add.text(panelW / 2 - 30, -135, '✖', { fontSize: '32px', color: '#fbcfe8' }).setOrigin(0.5).setInteractive();
    this.closeBtn.on('pointerup', () => this.closePopup());

    this.popupContainer.add([
      overlay, panel, this.popupTitle, ...this.popupStars, 
  ...
  openPopup(lvl, starsEarned, bestScore) {
    ...
    // Lock popup exactly to the center of the currently scrolled camera view
    ...
    this.tweens.add({ 
      targets: this.popupContainer, scale: 1, duration: 250, ease: 'Back.easeOut' 
    });
  }

  closePopup() {
    this.tweens.add({ 
      targets: this.popupContainer, scale: 0.7, duration: 150, ease: 'Quad.easeIn',
      onComplete: () => { 
